...
|---|---|---|---|
| **Local prototype** | JSON files (`playground/data/embeddings/<org>/`) | Standalone Python CLI | 🟢 Working |
...
- `playground/app/index_docs.py` — PDF → text → chunks → embeddings → JSON
- `playground/app/ask.py` — query → embed → cosine similarity → top-K chunks
